...
class AIService {
  ...
  Future<CodeReviewResult> analyzeCode(String code, String language) async {
    ...
    if (syntaxErrors.isNotEmpty) {
      final syntaxIssues = syntaxErrors.map((e) => CodeIssue(
        title: e.message,
        description: 'Line ${e.line}${e.column != null ? ', Column ${e.column}' : ''}: ${e.message}',
      ...
      final changedLines = syntaxErrors.map((e) => e.line).toSet().toList()..sort();
  ...
  Future<String> generateLesson(String level, String topic) async {
    ...
    try {
      ...
      return result['lesson']?['title'] ?? 'Generated lesson for $topic at $level level';
    } catch (e) {
  ...
  Future<Map<String, dynamic>> evaluateWriting(String userInput, String context) async {
    await Future.delayed(const Duration(seconds: 1));
    return {
      'score': 85,
      'feedback': 'Good use of technical terminology.',
      'corrections': [
        {'original': 'fix the bug', 'suggestion': 'resolve the defect', 'reason': 'More professional phrasing'},
      ],
  ...
  void _detectKeywordTypos(List<String> lines, List<SyntaxError> errors, String language) {
      ...
      for (final word in words) {
        ...
        if (typos.containsKey(word)) {
          errors.add(SyntaxError(
            ...
            description: 'The keyword "$word" is misspelled. The correct keyword is "${typos[word]}". This will cause a syntax or name error.',
          ));
        }
        // Check lowercase match (for case-sensitive typos)
          ...
          if (word.length <= 10 && !RegExp(r'[A-Z]').hasMatch(word.substring(1))) {
            errors.add(SyntaxError(
              line: i + 1,
              column: line.indexOf(word) + 1,
              ...
              description: 'The keyword "$word" is misspelled. The correct keyword is "$correctWord".',
            ));
          }
        }
      }
  ...
  void _detectPythonSyntax(List<String> lines, List<SyntaxError> errors) {
    for (int i = 0; i < lines.length; i++) {
      ...
      if (colonPat.hasMatch(trimmed)) {
        ...
        if (!stripped.endsWith(':') && stripped.isNotEmpty) {
          ...
          errors.add(SyntaxError(
            ...
            code: trimmed,
            fix: '$stripped:',
            description: 'In Python, all "$keyword" statements must end with a colon (:). '
      ...
      if (RegExp(r'^[a-zA-Z_]\w*\s*==\s*.+').hasMatch(trimmed) &&
          ...
          !trimmed.startsWith('assert') && !trimmed.contains('(')) {
        ...
        errors.add(SyntaxError(
          ...
          description: '"==" is a comparison operator (checks equality), while "=" is assignment (sets a value). '
      ...
      if (line.contains('\t') && line.contains('  ')) {
        errors.add(SyntaxError(
          ...
          description: 'Python requires consistent indentation. Mixing tabs and spaces will cause an IndentationError. '
  ...
  void _detectJSSyntax(List<String> lines, List<SyntaxError> errors, String lang) {
    for (int i = 0; i < lines.length; i++) {
      final line = lines[i];
      final trimmed = line.trimLeft().trimRight();
      final lineNum = i + 1;
      ...
      if (i + 1 < lines.length) {
        ...
        if (!trimmed.endsWith(',') && !trimmed.endsWith('{') &&
            ...
            !trimmed.endsWith(';') && !trimmed.endsWith('}') &&
            trimmed.contains(':') && nextTrimmed.contains(':') &&
          ...
          errors.add(SyntaxError(
            ...
            code: trimmed,
            fix: '$trimmed,',
            description: 'In objects and arrays, each item must be separated by a comma. '
      ...
      if (RegExp(r'^(let|const|var)\s+\w+\s*===').hasMatch(trimmed)) {
        errors.add(SyntaxError(
          ...
          code: trimmed,
          fix: trimmed.replaceFirst('===', '='),
          description: '"===" is strict comparison, "=" is assignment. You likely meant to assign a value here.',
        ));
      }
    }
  }
  ...
  bool _jsNeedsSemicolon(String trimmed) {
    ...
    if (trimmed.startsWith('if') || trimmed.startsWith('else') ||
        ...
        trimmed.startsWith('finally') || trimmed.startsWith('class') ||
    ...
    if (trimmed.startsWith('function') || trimmed.startsWith('async function') ||
        RegExp(r'^(const|let|var)\s+\w+\s*=\s*(async\s+)?\(').hasMatch(trimmed) ||
    ...
    if (trimmed.startsWith('const ') || trimmed.startsWith('let ') ||
        ...
        trimmed.startsWith('throw ') || trimmed.startsWith('await ') ||
        RegExp(r'^\w').hasMatch(trimmed)) {
      // But only if this line looks like it should be a statement
      if (trimmed.endsWith(')') || trimmed.endsWith('"') || trimmed.endsWith("'") ||
          trimmed.endsWith('`') || RegExp(r'[\w\d\]\)]$').hasMatch(trimmed)) {
        return true;
      }
    }
  ...
  void _detectDartSyntax(List<String> lines, List<SyntaxError> errors) {
    for (int i = 0; i < lines.length; i++) {
      ...
      if (!trimmed.endsWith(';') && !trimmed.endsWith('{') &&
          ...
          !trimmed.startsWith('part ') && !trimmed.startsWith('library')) {

        // Check if this line SHOULD have a semicolon
        if (trimmed.startsWith('return') || trimmed.startsWith('final ') ||
            trimmed.startsWith('var ') || trimmed.startsWith('const ') ||
            trimmed.startsWith('late ') || trimmed.startsWith('throw ') ||
            trimmed.startsWith('await ') || trimmed.startsWith('print(') ||
            trimmed.startsWith('debugPrint(') ||
            trimmed.contains(' = ') || trimmed.contains(' += ') ||
            trimmed.contains(' -= ') || trimmed.contains(' *= ') ||
            RegExp(r'^(int|double|String|bool|List|Map|Set|void|dynamic|num)\s').hasMatch(trimmed) ||
          ...
          errors.add(SyntaxError(
            ...
            code: trimmed,
            fix: '$trimmed;',
            description: 'In Dart, all statements must end with a semicolon (;). This is required, not optional.',
          ));
        }
      }
    }
  ...
  void _detectJavaCSharpSyntax(List<String> lines, List<SyntaxError> errors, String lang) {
    for (int i = 0; i < lines.length; i++) {
      final line = lines[i];
      final trimmed = line.trimLeft().trimRight();
      final lineNum = i + 1;
      ...
      if (!trimmed.endsWith(';') && !trimmed.endsWith('{') &&
        ...
        if (trimmed.startsWith('return') || trimmed.contains(' = ') ||
            ...
            RegExp(r'^(int|String|double|float|boolean|char|long|short|byte|void|var|final|static)\s').hasMatch(trimmed) ||
  ...
  void _detectCppSyntax(List<String> lines, List<SyntaxError> errors) {
    for (int i = 0; i < lines.length; i++) {
      ...
      if (!trimmed.endsWith(';') && !trimmed.endsWith('{') &&
          ...
          !trimmed.startsWith('namespace') && !trimmed.startsWith('template')) {

        if (trimmed.startsWith('return') || trimmed.contains(' = ') ||
            trimmed.startsWith('throw ') || trimmed.startsWith('delete ') ||
            RegExp(r'^(int|float|double|char|bool|void|auto|string|long|short|unsigned)\s').hasMatch(trimmed) ||
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (trimmed.endsWith(';') && !trimmed.contains('for')) {
        errors.add(SyntaxError(
          ...
          code: trimmed, fix: trimmed.substring(0, trimmed.length - 1),
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (trimmed.startsWith('guard') && !trimmed.contains('else')) {
        errors.add(SyntaxError(
          ...
          code: trimmed, fix: '$trimmed else { return }',
  ...
  void _detectRustSyntax(List<String> lines, List<SyntaxError> errors) {
    for (int i = 0; i < lines.length; i++) {
      ...
      if (!trimmed.endsWith(';') && !trimmed.endsWith('{') &&
          !trimmed.endsWith('}') && !trimmed.endsWith(',') &&
          !trimmed.startsWith('fn') && !trimmed.startsWith('if') &&
          !trimmed.startsWith('else') && !trimmed.startsWith('for') &&
          !trimmed.startsWith('while') && !trimmed.startsWith('match') &&
          !trimmed.startsWith('struct') && !trimmed.startsWith('enum') &&
          !trimmed.startsWith('impl') && !trimmed.startsWith('use') &&
          !trimmed.startsWith('mod') && !trimmed.startsWith('pub')) {
        if (trimmed.startsWith('let') || trimmed.startsWith('mut') ||
            trimmed.startsWith('return') || trimmed.contains(' = ')) {
          errors.add(SyntaxError(
            line: i + 1, column: trimmed.length + 1,
            message: 'Missing semicolon at end of statement',
            code: trimmed, fix: '$trimmed;',
  ...
  void _detectPhpSyntax(List<String> lines, List<SyntaxError> errors) {
    for (int i = 0; i < lines.length; i++) {
      ...
      if (trimmed.isEmpty || trimmed.startsWith('//') || trimmed.startsWith('#') ||
          trimmed.startsWith('/*') || trimmed.startsWith('*') ||
          trimmed == '<?php' || trimmed == '?>' || trimmed == '}' || trimmed == '{') {
        continue;
      }
      // Missing semicolons
      if (!trimmed.endsWith(';') && !trimmed.endsWith('{') &&
        ...
        if (trimmed.contains(' = ') || trimmed.startsWith('return') ||
            trimmed.startsWith('echo') || trimmed.startsWith('\$') ||
            RegExp(r'^\w+\(').hasMatch(trimmed)) {
          errors.add(SyntaxError(
            line: i + 1, column: trimmed.length + 1,
            message: 'Missing semicolon at end of statement',
  ...
  void _detectUnbalancedBrackets(List<String> lines, List<SyntaxError> errors) {
    ...
    if (parens > 0) {
      errors.add(SyntaxError(
        ...
        description: 'Every opening "(" must have a matching ")". Check function calls and expressions.',
      ));
    } else if (parens < 0) {
      errors.add(SyntaxError(
        line: lines.length, column: null,
        message: 'Extra closing parenthesis — ${-parens} unmatched ")"',
        code: lines.last.trim(), fix: lines.last.trim(),
        description: 'Found a closing ")" without a matching "(". Remove it or add the missing "(".',
      ));
    }
    ...
    if (brackets > 0) {
      errors.add(SyntaxError(
        ...
        description: 'Every "[" must have a matching "]". Check arrays, lists, or index operations.',
      ));
    }
    ...
    if (braces > 0) {
      errors.add(SyntaxError(
        ...
        description: 'Every "{" must have a matching "}". Check functions, classes, or code blocks.',
      ));
    } else if (braces < 0) {
      errors.add(SyntaxError(
        line: lines.length, column: null,
    ...
    for (final entry in errorsByLine.entries) {
      ...
      if (lineIdx >= 0 && lineIdx < lines.length) {
        ...
        if (lines[lineIdx].contains(err.code)) {
          ...
          final indent = lines[lineIdx].length - lines[lineIdx].trimLeft().length;
  ...
  List<CodeIssue> _generateIssues(String code, String language) {
    ...
    // Division by zero
    for (int i = 0; i < lines.length; i++) {
      if (lines[i].contains('/') && !lines[i].contains('//') && !lines[i].contains('http') &&
          (lines[i].contains('/ ') || lines[i].contains('/y') || lines[i].contains('/b'))) {
        if (!code.contains('== 0') && !code.contains('!= 0')) {
          issues.add(CodeIssue(
            ...
            severity: IssueSeverity.critical, type: IssueType.bug, lineNumber: i + 1,
            suggestion: 'Add a zero-check before performing division.',
            codeExample: 'if (divisor == 0) throw ArgumentError("Cannot divide by zero");',
            explanation: 'Division by zero causes a runtime error in most languages.',
    ...
    // var usage
    if (code.contains('var ') && (language == 'Dart' || language == 'JavaScript' || language == 'TypeScript')) {
      ...
      issues.add(CodeIssue(
        ...
        simpleExplanation: '💡 Using specific types makes your code easier to understand.',
        severity: IssueSeverity.warning, type: IssueType.style, lineNumber: lineIndex >= 0 ? lineIndex + 1 : 1,
        suggestion: 'Replace var with the specific type.',
        explanation: 'Explicit types catch type-related bugs at compile time.',
    ...
    if (code.contains('print(') || code.contains('console.log(')) {
      final lineIndex = lines.indexWhere((l) => l.contains('print(') || l.contains('console.log('));
      issues.add(CodeIssue(
        ...
        severity: IssueSeverity.warning, type: IssueType.bestPractice,
    ...
    if (!code.contains('try') && !code.contains('catch') && !code.contains('except')) {
      issues.add(CodeIssue(
        ...
        severity: IssueSeverity.error, type: IssueType.security,
        suggestion: 'Wrap risky operations in try-catch blocks.',
        explanation: 'Without error handling, a single failure can crash your application.',
    ...
    if (code.contains('" +') || code.contains("' +") || code.contains('+ "') || code.contains("+ '")) {
      final lineIndex = lines.indexWhere((l) => l.contains('" +') || l.contains("' +") || l.contains('+ "') || l.contains("+ '"));
      issues.add(CodeIssue(
        ...
        severity: IssueSeverity.warning, type: IssueType.performance,
        lineNumber: lineIndex >= 0 ? lineIndex + 1 : null,
    ...
    if (code.contains('TODO') || code.contains('FIXME')) {
      final lineIndex = lines.indexWhere((l) => l.contains('TODO') || l.contains('FIXME'));
      issues.add(CodeIssue(
        ...
        severity: IssueSeverity.info, type: IssueType.bestPractice,
        lineNumber: lineIndex >= 0 ? lineIndex + 1 : null,
    ...
    // Hardcoded secrets
    if (RegExp(r'["\x27](password|secret|api[_-]?key|token)["\x27]', caseSensitive: false).hasMatch(code)) {
      issues.add(CodeIssue(
        title: 'Hardcoded sensitive data detected',
        description: 'Passwords, API keys, or tokens appear to be hardcoded.',
        simpleExplanation: '💡 Never put passwords or API keys directly in your code!',
        severity: IssueSeverity.critical, type: IssueType.security,
        suggestion: 'Use environment variables or a secrets manager.',
        explanation: 'If pushed to a public repo, anyone can see your secrets.',
    ...
    if (issues.isEmpty) {
      issues.add(CodeIssue(
        ...
        explanation: 'Without validation, invalid inputs can cause subtle bugs.',
      ));
    }
  ...
  String _generateFixedCode(String code, String language) {
    ...
    if (!code.contains('try') && !code.contains('catch') && !code.contains('except')) {
      if (language == 'Python') {
        ...
        int funcLine = -1;
        for (int i = 0; i < lines.length; i++) {
          if (funcMatch.hasMatch(lines[i])) { funcLine = i; break; }
        }
        if (funcLine >= 0) {
          final body = lines.sublist(funcLine + 1).map((l) => '    $l').join('\n');
          fixed = '${lines.sublist(0, funcLine + 1).join('\n')}\n    try:\n$body\n    except Exception as e:\n        print(f"Error: {e}")\n        return None';
        }
      } else if (language == 'Dart' || language == 'JavaScript' || language == 'TypeScript') {
        fixed = fixed.replaceFirst('{', '{\n  try {');
        if (fixed.contains('}')) {
          final lastBrace = fixed.lastIndexOf('}');
          fixed = '${fixed.substring(0, lastBrace)}  } catch (e) {\n    debugPrint("Error: \$e");\n    rethrow;\n  }\n}';
        }
      }
    }
    if (language == 'Dart') {
  ...
  List<int> _getChangedLines(String code) {
    ...
    final changed = <int>[];
    for (int i = 0; i < lines.length; i++) {
      if (lines[i].contains('var ') || lines[i].contains('print(') ||
          lines[i].contains('console.log(') || lines[i].contains('" +') ||
          lines[i].contains('TODO') || (lines[i].contains('/') && !lines[i].contains('//'))) {
        changed.add(i + 1);
      }
    }
  ...
  Map<String, int> _generateRatings(List<CodeIssue> issues, int score) {
    ...
    for (final issue in issues) {
      switch (issue.type) {
        case IssueType.security: security -= (issue.severity == IssueSeverity.critical ? 30 : 15); break;
        case IssueType.performance: performance -= (issue.severity == IssueSeverity.critical ? 25 : 10); break;
        case IssueType.style: case IssueType.bestPractice: maintainability -= 8; break;
        case IssueType.bug: quality -= 10; break;
        case IssueType.syntax: break;
      }
    }
    return {
  ...
  String _generateSummary(String code, String language, List<CodeIssue> issues) {
    final critCount = issues.where((i) => i.severity == IssueSeverity.critical).length;
    final highCount = issues.where((i) => i.severity == IssueSeverity.error).length;
    if (critCount > 0) {
  ...
  int _calculateScore(List<CodeIssue> issues) {
    ...
    for (final issue in issues) {
      switch (issue.severity) {
        case IssueSeverity.critical: score -= 20; break;
        case IssueSeverity.error: score -= 15; break;
        case IssueSeverity.warning: score -= 8; break;
        case IssueSeverity.info: score -= 3; break;
      }
    }
