...
print(data)''',
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Code Review', style: GoogleFonts.inter(fontSize: 22, fontWeight: FontWeight.w700)),
        ...
        Container(
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            const Icon(Icons.rate_review_rounded, color: AppColors.primary, size: 16),
            const SizedBox(width: 4),
            Text('${provider.user.codeReviewsCompleted}',
  ...
  Widget _buildLanguageSelector(AppProvider provider, bool isDark) {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Programming Language', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600)),
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    color: isSelected ? null : (isDark ? AppColors.darkCard : AppColors.lightSurface),
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Your Code', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600)),
          Row(children: [
            ...
            GestureDetector(
              onTap: () async {
                ...
                if (data?.text != null) {
                  ...
                  if (detected != null) {
                    context.read<AppProvider>().setSelectedLanguage(detected);
                  }
                }
              },
              child: Container(
                ...
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  SizedBox(width: 4),
                  Text('Paste', style: TextStyle(fontSize: 12, color: AppColors.primary, fontWeight: FontWeight.w600)),
                ]),
              ),
            ),
        ...
        Container(
          ...
          child: Column(children: [
            ...
            Container(
              ...
              child: Row(children: [
                ...[0xFFFF5555, 0xFFFFB86C, 0xFF50FA7B].map((hex) => Container(
                  width: 10, height: 10, margin: const EdgeInsets.only(right: 6),
                  decoration: BoxDecoration(shape: BoxShape.circle, color: Color(hex)),
                )),
                const SizedBox(width: 8),
                Text('${provider.selectedLanguage.toLowerCase()}_code.${_getExt(provider.selectedLanguage)}',
                    style: GoogleFonts.firaCode(fontSize: 12, color: AppColors.darkTextSecondary)),
                const Spacer(),
                Icon(Icons.fullscreen_rounded, size: 18, color: AppColors.darkTextSecondary.withValues(alpha: 0.5)),
              ]),
            ),
            // Code input
            ConstrainedBox(
              ...
              child: TextField(
                ...
                maxLines: null,
                minLines: 8,
                style: GoogleFonts.firaCode(fontSize: 13, color: AppColors.darkText, height: 1.6),
                decoration: InputDecoration(
                  filled: true,
                  fillColor: AppColors.codeBackground,
                  hintText: 'Paste your code here...',
                  hintStyle: GoogleFonts.firaCode(color: AppColors.darkTextSecondary.withValues(alpha: 0.4), fontSize: 13),
  ...
  Widget _buildQuickExamples(AppProvider provider, bool isDark) {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Icon(Icons.lightbulb_rounded, color: AppColors.xpColor, size: 18),
          const SizedBox(width: 6),
          Text('Quick Examples', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600)),
        ]),
        const SizedBox(height: 10),
        Wrap(
          spacing: 8,
  ...
  Widget _buildAnalyzingState(bool isDark) {
    return AnimatedBuilder(
      ...
      builder: (context, child) => Container(
        ...
        decoration: BoxDecoration(
          color: AppColors.primary.withValues(alpha: 0.05 + (_pulseController.value * 0.05)),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(color: AppColors.primary.withValues(alpha: 0.2 + (_pulseController.value * 0.15))),
        ),
        child: Column(children: [
          const SizedBox(
            width: 48, height: 48,
          ...
          Text('Analyzing Your Code...', style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.primary)),
          const SizedBox(height: 16),
            ...
            return Padding(
              ...
              child: Row(children: [
                AnimatedSwitcher(
                  ...
                  child: isComplete
                      ? const Icon(Icons.check_circle_rounded, color: Color(0xFF00D68F), size: 20, key: ValueKey('done'))
                      : isCurrent
                          ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2, valueColor: AlwaysStoppedAnimation(AppColors.primary)))
                          : Icon(Icons.circle_outlined, color: Colors.grey.shade400, size: 20, key: const ValueKey('pending')),
                ),
  ...
  Widget _buildTips(bool isDark) {
    return Padding(
      ...
      child: GlassCard(
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            const Icon(Icons.tips_and_updates_rounded, color: AppColors.xpColor, size: 20),
            const SizedBox(width: 8),
            Text('Pro Tips', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600)),
          ]),
          ...
          ...['Paste your actual project code for best results',
              ...
              'Check the vocabulary section for new tech terms'].map((t) => Padding(
            padding: const EdgeInsets.only(bottom: 6),
            child: Row(children: [
              Container(width: 4, height: 4, decoration: const BoxDecoration(shape: BoxShape.circle, color: AppColors.primary)),
              const SizedBox(width: 10),
              Expanded(child: Text(t, style: TextStyle(fontSize: 13,
                  color: isDark ? AppColors.darkTextSecondary : AppColors.lightTextSecondary))),
            ]),
          )),
        ]),
  ...
  Widget _buildHistory(AppProvider provider, bool isDark) {
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Text('Recent Reviews', style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w700)),
          const Spacer(),
          Text('${provider.reviewHistory.length} total',
              style: TextStyle(fontSize: 12, color: isDark ? AppColors.darkTextSecondary : AppColors.lightTextSecondary)),
        ]),
        const SizedBox(height: 8),
        ...provider.reviewHistory.take(5).map((review) => GlassCard(
          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => CodeResultScreen(result: review))),
          ...
          child: Row(children: [
            Container(
              ...
              child: Center(child: Text('${review.overallScore}', style: TextStyle(
                color: review.scoreColor, fontWeight: FontWeight.w800, fontSize: 16))),
            ),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${review.language} Code Review', style: GoogleFonts.inter(fontSize: 14, fontWeight: FontWeight.w600)),
              Text('${review.issues.length} issues · Score: ${review.overallScore}/100',
                  style: TextStyle(fontSize: 12, color: isDark ? AppColors.darkTextSecondary : AppColors.lightTextSecondary)),
            ])),
  ...
  Future<void> _analyzeCode(BuildContext context) async {
    ...
    try {
      ...
      if (mounted) {
        Navigator.push(context, MaterialPageRoute(builder: (_) => CodeResultScreen(result: result)));
      }
  ...
  String? _detectLanguage(String code) {
    const signatures = <String, List<String>>{
      'C++': ['#include', 'std::', 'cout', 'cin', 'using namespace',
              'int main(', 'endl'],
      'Dart': ["import 'package:", 'void main(', '@override',
               'StatelessWidget', 'StatefulWidget', 'BuildContext',
               'Widget ', 'setState('],
      'Python': ['def ', 'elif ', "if __name__ == '__main__':",
                 'self.', '__init__', 'import ', 'from ', 'print('],
      'Java': ['public class', 'System.out.println', 'package ',
               'import java.', '@Override', 'void main(String'],
      'JavaScript': ['console.log', 'function ', 'const ', 'let ',
                     'document.', 'require(', 'module.exports', '=>'],
      'TypeScript': ['interface ', ': string', ': number', ': boolean',
                     'import {', 'export ', 'type '],
      'Kotlin': ['fun ', 'val ', 'println(', 'suspend ',
                 'override fun', 'companion object', 'data class'],
      'Swift': ['func ', 'import UIKit', 'import Foundation',
                'guard ', '@IBOutlet', 'override func'],
      'Go': ['package main', 'func main()', 'fmt.', 'import (',
             ':= ', 'func ('],
      'Rust': ['fn main()', 'let mut', 'println!', 'use std::',
               'impl ', 'pub fn'],
      'C#': ['using System', 'namespace ', 'Console.Write',
             'static void Main', 'get;', 'set;'],
