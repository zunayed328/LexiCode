...
class _WritingPracticeScreenState extends State<WritingPracticeScreen> {
  ...
  Widget _buildWritingScreen() {
    ...
    final wordCount = _textController.text.trim().split(RegExp(r'\s+')).where((s) => s.isNotEmpty).length;
    
    return Scaffold(
      appBar: AppBar(
        title: Text('Writing Lab', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
          ...
          Center(
            child: Padding(
              ...
              child: Text(
                ...
                style: GoogleFonts.inter(fontWeight: FontWeight.w600, color: wordCount >= _task!.wordCountTarget ? AppColors.success : (isDark ? Colors.white70 : Colors.black54)),
              ),
            ),
          ),
        ],
            ...
            children: [
              GlassCard(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text('Prompt', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w700, color: const Color(0xFFEC4899).withValues(alpha: 0.8))),
                      ...
                      if (_task!.suggestedVocabulary.isNotEmpty) ...[
                        ...
                        Text('Suggested Vocabulary:', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600)),
                        const SizedBox(height: 4),
                        Wrap(
                          spacing: 8,
                          runSpacing: 8,
                          children: _task!.suggestedVocabulary.map((v) => Container(
                            ...
                            child: Text(v, style: GoogleFonts.inter(fontSize: 12, color: const Color(0xFFEC4899))),
                          )).toList(),
                        ),
                      ],
                    ],
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: _isEvaluating || _textController.text.trim().isEmpty ? null : _evaluateWriting,
  ...
  Widget _buildEvaluationScreen() {
    ...
    final scorePercent = (_evaluation!.overallBand / 9.0 * 100).clamp(0.0, 100.0);
    
    return Scaffold(
      appBar: AppBar(
        title: Text('Results', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
        backgroundColor: Colors.transparent,
        elevation: 0,
      ),
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('Detailed Feedback', style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w800)),
              const SizedBox(height: 12),
              GlassCard(
                child: Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: Text(
                    _evaluation!.detailedFeedback ?? 'No detailed feedback available.',
              ...
              if (_evaluation!.strengths.isNotEmpty) ...[
                const SizedBox(height: 24),
                Text('Strengths', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.success)),
                const SizedBox(height: 8),
                ..._evaluation!.strengths.map((s) => Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Icon(Icons.check_circle_rounded, color: AppColors.success, size: 18),
                    const SizedBox(width: 8),
                    Expanded(child: Text(s, style: GoogleFonts.inter(height: 1.4))),
                  ],
                )),
              ],
              if (_evaluation!.weaknesses.isNotEmpty) ...[
                const SizedBox(height: 24),
                Text('Areas for Improvement', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.error)),
                const SizedBox(height: 8),
                ..._evaluation!.weaknesses.map((w) => Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Icon(Icons.arrow_upward_rounded, color: AppColors.error, size: 18),
                    const SizedBox(width: 8),
                    Expanded(child: Text(w, style: GoogleFonts.inter(height: 1.4))),
                  ],
                )),
              ],
              if (_evaluation!.grammarErrors.isNotEmpty) ...[
                const SizedBox(height: 24),
                Text('Corrections', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700)),
                const SizedBox(height: 12),
                ..._evaluation!.grammarErrors.map((e) => Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(e.original, style: GoogleFonts.inter(color: AppColors.error, decoration: TextDecoration.lineThrough)),
                      const SizedBox(height: 4),
                      Text(e.correction, style: GoogleFonts.inter(color: AppColors.success, fontWeight: FontWeight.w600)),
                      const SizedBox(height: 4),
                      Text(e.explanation, style: GoogleFonts.inter(fontSize: 12, color: isDark ? Colors.white70 : Colors.black54)),
                    ],
                  ),
                )),
              ],
